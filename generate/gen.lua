...
-- ZZZ _cpp and _ppss are generated by calc.lua

-- Cost per power per reactor level, from 1 to 12
local _cpp = {
	0.31829004329004,
	0.29534451198606,
	0.27239898068207,
...
-- Power per small utility slot, from 1 to 12
local _ppss = {
	23.645833333333,
	36.708333333333,
	49.770833333333,
...
--local _sets = { 7, 12, 16, 18, 20, 24, 30, 36, 40, 48, 60, 64, 72, 76, 84 }
...
utility_component_template = {
	...
	for _, supplier in next, _REACTORS do
		...
		for _, data in next, add do
			data.powerPerSmallSlot = _ppss[data.level]
			data.costPerPower = _cpp[data.level]
			_reactorData[#_reactorData+1] = data
		end
	end
	table.sort(_reactorData, function(a, b)
		if a.powerPerSmallSlot == b.powerPerSmallSlot then return a.costPerPower < b.costPerPower end
		return a.powerPerSmallSlot < b.powerPerSmallSlot
	end)

	local function calculatePower(slots, power)
		local r = math.floor(slots * power)
		local m = r % 5
		if m == 0 then return r end
		return r - 5 - m
	end
	local function calculateCost(power, cost)
		local r = math.ceil(power * cost)
		local m = r % 5
		if m == 0 then return r end
		return r + 5 - m
	end
